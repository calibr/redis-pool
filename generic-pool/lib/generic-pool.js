...
exports.Pool = function(factory) {
  var me = {}, idleTimeoutMillis = factory.idleTimeoutMillis || 30000, reapInterval = factory.reapIntervalMillis || 1000, refreshIdle = ('refreshIdle' in factory) ? factory.refreshIdle : true, availableObjects = [], // objects that available for acquire
  allObjects = [], // all objects in pool, raw objects

  waitingClients = new PriorityQueue(factory.priorityRange || 1), count = 0, removeIdleScheduled = false, removeIdleTimer = null, draining = false,
  ...
  log = factory.log ? (function(str, level) {
    ...
    if ( typeof factory.log === 'function') {
      ...
      console.log(level + " pool " + factory.name + " - " + str);
    }
  }
  ) : function() {
  };
  ...
  me.destroy = function(obj) {
    ...
    allObjects = allObjects.filter(function(_obj) {
      return (_obj !== obj);
    });
  ...
  function removeIdle() {
    ...
    log("Start remove idle");

    var toRemove = [], now = new Date().getTime(), i, al, tr, timeout;
    ...
    for ( i = 0, al = availableObjects.length; i < al && (refreshIdle || (count - factory.min > toRemove.length)); i += 1) {
      ...
      log(availableObjects[i].obj.__id + " TTL " + timeout - now);
   ...
   */
  function dispense() {
    var obj = null, objWithTimeout = null, err = null, clientCb = null, waitingCount = waitingClients.size();
  ...
  me.acquire = function(callback, priority) {
    ...
    waitingClients.enqueue(function(error, obj) {

      obj.__poolStartUse = new Date();
  ...
  me.release = function(obj) {
    ...
    if (obj.__debugInfo) {
      log("---RELEASE CONNECTION ", obj.__debugInfo);
    }
    ...
    if (availableObjects.some(function(objWithTimeout) {
      ...
      console.warn("release called twice for the same resource: " + (new Error().stack), 'error');
      return;
    }
    //log("return to pool");
    var objWithTimeout = {
    ...
    log(obj.__id + " timeout: " + objWithTimeout.timeout, 'verbose');
    dispense();
    scheduleRemoveIdle();
  };
  ...
  me.getObjectsInUse = function() {
    return allObjects.filter(function(obj) {
      return obj.__poolInUse;
    });
  }

  me.getAllObjects = function() {
    return allObjects;
  }
  ...
  me.getAvailableObjects = function() {
    ...
    availableObjects.forEach(function(timedObj) {
      r.push(timedObj.obj);
    });
